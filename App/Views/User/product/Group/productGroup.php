...
<?php else : ?>
  ...
  <div class="image-gallery flex-row gap20 w100">
    <div class="gallery-main">
      <?php foreach ($products as $pIdx => $prod) :
        ...
        if (trim($pTitle) === '' && trim($pUrl) === '#') continue;
        ?>
        <div class="gallery-item p10">
          <div class="theme-button pointer p7 <?= ($card["borders"][0] == "br50") ? "br20" : $card["borders"][0] ?> <?= $card["shadow"] ?> ">
            <a href="<?= e($pUrl) ?>" target="_blank" class="track-link-click flex-column gap5 w100 h100" data-user="<?= e($profile) ?>" data-link-id="<?= e($pUrl) ?>" style="text-decoration: none; color: inherit;">
              <?php if ($pImgShow && $hasImg) : ?>
                <figure class="w100 ar-square overflow-hidden">
                  <img src="<?= e($pImgSrc) ?>" alt="<?= e($pTitle) ?>" class="cover w100 h100 <?= ($card["borders"][1] == "br50") ? "br12" : $card["borders"][1] ?>" fetchpriority="high">
                </figure>
