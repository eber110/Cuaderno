      ...
      <div class="|product-grid-card theme-button pointer flex-column between-stretch p7 gap8 <?= ($card["borders"][0] == "br50") ? "br20" : $card["borders"][0] ?> <?= $card["shadow"] ?> position-relative" style="overflow: hidden;">
        <a href="<?= e($pUrl) ?>" target="_blank" class="track-link-click flex-column gap8 w100 h100" data-user="<?= e($profile) ?>" data-link-id="<?= e($pUrl) ?>" style="text-decoration: none; color: inherit;">
          ...
          <div class="flex-column gap4 w100 flex-1">
            <p class="bold500 w100 capitalize-p cut-phrase" cant-col="2" style="line-height: 1.25;">
              <?= svg("basket-shopping", "x16") ?> <?= e($pTitle) ?>
            </p>
...
<?php else : ?>
  ...
  <div class="product-group-slide w100" style="display: flex; flex-direction: row; gap: 12px; overflow-x: auto; scroll-snap-type: x mandatory; -webkit-overflow-scrolling: touch; padding-bottom: 6px;">
    <?php foreach ($products as $pIdx => $prod) :
      $pTitle      = $prod["title"] ?? '';
      $pUrl        = $prod["url"] ?? '#';
      $pPrice      = $prod["price"] ?? '';
      ...
      if (trim($pTitle) === '' && trim($pUrl) === '#') continue;
    ?>
      <div class="|product-slide-card theme-button pointer flex-column between-stretch p7 gap8 <?= ($card["borders"][0] == "br50") ? "br20" : $card["borders"][0] ?> <?= $card["shadow"] ?> position-relative" style="flex: 0 0 200px; width: 200px; scroll-snap-align: start; overflow: hidden;">
        <a href="<?= e($pUrl) ?>" target="_blank" class="track-link-click flex-column gap8 w100 h100" data-user="<?= e($profile) ?>" data-link-id="<?= e($pUrl) ?>" style="text-decoration: none; color: inherit;">
          <?php if ($pImgShow && $hasImg) : ?>
            <figure class="w100 ar-square overflow-hidden">
              <img src="<?= e($pImgSrc) ?>" alt="<?= e($pTitle) ?>" class="cover w100 h100 <?= ($card["borders"][1] == "br50") ? "br12" : $card["borders"][1] ?>" fetchpriority="high">
          ...
          <div class="flex-column gap4 w100 flex-1">
            ...
            <div class="mt-auto">
              <?php if (!$pOffer || empty($pDiscount)) : ?>
                <?php if ($pPrice !== '') : ?>
                  <p class="bold600 ">$<?= e($pPrice) ?></p>
                <?php endif; ?>
              <?php else : ?>
                <div class="flex-column gap0">
                  <p class="inactive x16" style="text-decoration: line-through;">$<?= e($pPrice) ?></p>
                  <p class="bold600 text-success">$<?= e($pDiscount) ?></p>
                </div>
              <?php endif; ?>
            </div>
          </div>
