<header class="container observer container-xl-mid container-sml flex-row center-center flex-column-sml center-center-sml gap-sml-30 h-dvh-desk pb100 pt-mid-100 pb-mid-100 pt-sml-50 pb-sml-50">
  <div class="hem20 w100 p15 p-sml-0 flex-column center-start">
    <div class="flex-column gap5">
      <p class="oswald bold600 x28 color5 slide-in-bottom ob-0 dl-100">Rendimiento sin fricción</p>
      <h1 class="oswald bold700 capitalize-p line-h10 x80 x-mid-50 x-sml-40 color5 slide-in-bottom ob-0 dl-200 m0 p0">Tu contenido centralizado. Cargando al instante.</h1>
      <p class="bold500 x20 x-sml-18 slide-in-bottom ob-0 dl-400 pt20 pb25 pt-mid-10 pb-mid-20 pt-sml-5">Reúne tus productos, eventos, enlaces y campañas en un hub ultraligero. Configúralo en segundos y ofrece a tu audiencia una experiencia libre de esperas.</p>
    </div>
    <a href="/registrar" class="slide-in-bottom ob-0 dl-600 p10 pr30 pl30 br50 w-auto back5-hover textc x20 x-sml-18 bold500 pointer">
      <p>Crear mi Clikhub gratis</p>
    </a>
  </div>

  <div class="w100 flex-column center-center m-mid-15">
    <div class="wpx400 wpx-mid-350 w-sml-50 hpx800 hpx-mid-800 hpx-sml-420 slide-in-right ob-0 dl-300">
      <img src="<?= DIR_UPLOAD_MEDIA_STATIC."hero.webp"?>" alt="" class="animate-img-hero1 absolute hover-lift-ns">
      <img src="<?= DIR_UPLOAD_MEDIA_STATIC."hero2.webp"?>" alt="" class="animate-img-hero2 absolute hover-lift-ns">
      <img src="<?= DIR_UPLOAD_MEDIA_STATIC."hero3.webp"?>" alt="" class="animate-img-hero3 absolute hover-lift-ns">
    </div>
  </div>
</header>